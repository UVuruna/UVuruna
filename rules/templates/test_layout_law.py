...
TEMPLATE. Copy to a project's `tests/test_layout_law.py` and adjust only the
CONFIGURATION section. It is the fifth guard test, and it runs for GUI projects
only (rules/CODE.md -> Enforcement).

What it fails on: GUI sources that cut content off, or that freeze an element so
it cannot take the window's free space - elide/trim APIs, forced scrollbars,
disabled wrapping, hard pixel sizes on text-bearing widgets.
...
GUI_DIRS = ("gui", "ui", "views", "widgets", "windows", "dialogs")
...
RATCHET: dict[str, str] = {
    # "gui/legacy_table.py": "elide on the vendored table header; owed by the
    #                         table rewrite - session 2026-08-05",
}
